...
import com.mac.m2land_cms.config.filter.CustomAuthenticationFilter;
import com.mac.m2land_cms.config.filter.CustomAuthorizationFilter;
...
public class SecurityConfiguration implements EnvironmentAware {
    ...
    public static String[] PUBLIC_OPERATION = {"userRegister","fakeLogin","fakeLogout"};
...
public CustomAuthorizationFilter authorizationFilter() throws Exception {
    ...
    public class MyCustomDsl extends AbstractHttpConfigurer<MyCustomDsl, HttpSecurity> {
        ...
        public void configure(HttpSecurity http) throws Exception {
            ...
            http.addFilter(new CustomAuthenticationFilter(authenticationManager, tokenService, firebaseMessagingService, gameLogRepository));
    ...
    public class GlobalHeaderOperationCustomizer implements OperationCustomizer {
        ...
        public Operation customize(Operation operation, HandlerMethod handlerMethod) {
            ...
            Parameter bearerHeader = new Parameter().in(ParameterIn.HEADER.toString()).name("prefix")
                    .description("Bearer prefix: required)").schema(new StringSchema()).example("Bearer").required(false);
...
//                            if (Arrays.stream(PUBLIC_OPERATION).anyMatch(s -> s.equals(operation.getOperationId()))) {
//                                operation.getParameters().removeIf(parameter1 -> "access-token".equals(parameter1.getName()));
//                                operation.getParameters().removeIf(parameter1 -> "refresh-token".equals(parameter1.getName()));
//                                operation.getParameters().removeIf(parameter1 -> "prefix".equals(parameter1.getName()));
//                            }
//                        }
//                );
//    }
